...
export default function BusinessSelector({ businesses, selectedId, onChange, loading }: BusinessSelectorProps) {
    ...
    return (
        <div className="relative" ref={ref}>
            <button
                onClick={() => setOpen((v) => !v)}
                className="flex items-center gap-2.5 border border-gray-200 rounded-lg pl-2 pr-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors min-w-[200px]"
            >
                {selected ? (
                ...
                <span className="flex-1 text-left truncate">
                    {loading ? 'Loading...' : selected ? selected.name : 'All Businesses'}
                </span>
            ...
            {open && (
                <div className="absolute left-0 top-full mt-1.5 z-20 w-64 bg-white border border-gray-100 rounded-xl shadow-lg py-1 max-h-80 overflow-y-auto">
                    <button
                        ...
                        className={`w-full flex items-center gap-2.5 text-left px-3 py-2 text-sm hover:bg-gray-50 ${
                            selectedId === 'all' ? 'text-blue-600 font-medium' : 'text-gray-700'
                        }`}
                    >
                        <span className="w-6 h-6 rounded-md bg-blue-50 text-blue-500 flex items-center justify-center shrink-0">
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
